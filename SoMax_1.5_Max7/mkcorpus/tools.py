from numpy import array, exp, where, log2, floor, ceil, zeros, log, arange, round, maximum, ones_like, power, dot, \
    transpose
    ...
    for i in range(0, nbNotes):
        ...
        if listOfMidiHarmonics.size != 0:
            pVector[listOfMidiHarmonics, ind_t_on:ind_t_off] = \
                maximum(pVector[listOfMidiHarmonics, ind_t_on:ind_t_off]
                        , dot(power(ones_like(listOfMidiHarmonics) * decayParam
                                    , arange(1, listOfMidiHarmonics.size + 1))
                              .reshape(listOfMidiHarmonics.size, 1), p_up.reshape(1, p_up.size)))

            pVector[listOfMidiHarmonics, ind_t_off:ind_t_end] = \
                maximum(pVector[listOfMidiHarmonics, ind_t_off:ind_t_end]
                        , dot(power(ones_like(listOfMidiHarmonics) * decayParam
                                    , arange(1, listOfMidiHarmonics.size + 1))
                              .reshape(listOfMidiHarmonics.size, 1), p_down.reshape(1, p_down.size)))
...
def getPitchContent(data, stateNb, legato):
    ...
    for k in range(0, nbNotesInSlice):
        if (data[stateNb]["notes"][k]["note"][1] > 0) or (data[stateNb]["notes"][k]["time"][0] > legato):
